...
Primeiramente incluímos a identificação do cluster no dataset original.

```{r data_load_cluster, echo=TRUE}
target_data <- readRDS('../data/processed/target_dataset_classificada.rds')

```

```{r data_cluster_1, echo=TRUE, out.width='100%', message=FALSE}

cluster_1 = dplyr::filter(target_data, cluster_id == 1)

plot_exploratory_analysis(cluster_1)
...
**Idade**: conforme podemos observar existe sim uma nítida diferença na idade média dos entrevistados classificados no cluster 2 em comparação com os demais clusters.

A idade do cluster 2 é de aproximadamente 4 anos de diferença.

```{r data_cluster_idade, echo=TRUE}
...
**Sexo**: conforme podemos observar existe sim uma nítida diferença na composição de sexo dos entrevistados classificados no cluster 1 em comparação com os demais clusters.

No cluster 1, 80% dos entrevistados são do sexo masculino, e nos demais clusters a proporção é mais equilibrada.

```{r data_cluster_sexo, echo=TRUE}
