---
title: "Modelos de Atribuição por Cadeias de Markov e comparação com outros modelos."
output:
  html_notebook: default
  html_document: default
---

Exemplos didáticos deste notebook foram extraídos ou baseados em: https://www.r-bloggers.com/attribution-model-with-r-part-1-markov-chains-concept/
...
Extraindo os resultados da atribuição.
...
Extraindo a matriz de transição.
...
Completando a matriz de transição (passo 1: extraindo a matriz de transição).
...
Completando a matriz de transição (passo 2: adicionando dummies).
...
Completando a matriz de transição (passo 2: fatorizando os estados e atribuindo de-para).
...
Adicionando zeros nas indefinições da matriz de transição e criandoo objeto markovchain.
...
Simulando dados reais de log de navegação.
...
Agregando a informação de canais por cada cliente. Para simplificar, assume-se que a conversão é 100% para todos os caminhos.
...
Calculando os modelos (Heurístico que servirá de base para os modelos de Primeiro Contato e Último Contato).

```{r}
# A função heuristic_models() não funcionou para o autor deste código, de modo que ele faz os cálculos manualmente.
...
Calculando os modelos (Último Contato).
...
Visualização do mapa de calor da matriz de transição.
...
Visualização do comparativo de modelos.
